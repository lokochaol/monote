...
final class MemoBlockPersistence {
	/// 現在の保存先ルート。iCloud 昇格に成功すると `<ubiquityContainer>/Documents/memo_blocks` を指す。
	/// 既定はローカル `Documents/memo_blocks/`。
	private(set) var rootURL: URL
	/// `rootURL` と同期する `index.json` の URL。`rootURL` が変わったら必ず差し替える。
	private(set) var indexURL: URL
	/// 現在のルートが iCloud Drive (Ubiquity Container) を指しているか。`true` のときは
	/// 書き込みに `NSFileCoordinator` を被せたり、未ダウンロードファイルへの配慮が必要。
	...
	/// `rootURL` を別の場所（典型的には iCloud Drive）へ差し替える。
	/// - 注意: 呼ぶタイミングは「ブロックの読み書きがまだ走っていない（または安全な間）」に限る。
	///   現状は起動直後の `prepareStorageRootIfNeeded()` からだけ叩く。
	private func relocateRoot(to newRoot: URL) {
		try? FileManager.default.createDirectory(at: newRoot, withIntermediateDirectories: true)
		rootURL = newRoot
		indexURL = newRoot.appendingPathComponent("index.json")
	}

	// MARK: - iCloud 昇格

	/// 起動時に呼ぶ: 設定で iCloud が有効、かつ Ubiquity Container が使えるなら、
	/// 1) ローカル → iCloud の取り残しがあれば毎回上げ直し、
	/// 2) `rootURL` を iCloud 側へ切り替える。
	/// - 「初回だけ移行する」式だと `setUbiquitous` が個別に失敗したファイルを永遠に取りこぼすので、
	///   起動毎に軽くリトライする方が信頼できる（中身が空ならほぼコスト 0）。
	/// - Returns: iCloud ルートに切り替わったとき `true`。
	@discardableResult
	func prepareStorageRootIfNeeded() async -> Bool {
		...
		// 重い `url(forUbiquityContainerIdentifier:)` と `setUbiquitous` は detached で。
		let resolvedICloudRoot = await Task.detached(priority: .userInitiated) { () -> URL? in
			...
			// ローカルに残っているファイルがあれば毎回上げ直す。1 度の昇格に失敗したアイテムが
			// あっても次回以降に拾い続けるための保険。
	...
	/// iCloud ルート上で使っている `block_*.json` と `attachments/*` のうち、
	/// 「`keepBlockIndices` で指定された範囲のブロック番号と、それらが参照する添付」以外を
	/// `evictUbiquitousItem(at:)` で能動的にローカル端末から退避させる。
	/// - iCloud にはファイルは残ったままで、端末上のキャッシュ容量だけが解放される。
	/// - ローカルルートで動いているとき (`!isUsingICloudRoot`) は何もしない。
	/// - Returns: 退避を試みたファイル件数（成功・失敗合算）。
	@discardableResult
	func evictUnusedICloudItems(keepBlockIndices: Set<Int>) async -> Int {
		guard isUsingICloudRoot else { return 0 }
		let rootURL = self.rootURL
		let referencedHashes = await collectReferencedAttachmentHashesAsync(forBlockIndices: keepBlockIndices)
		let count = await Task.detached(priority: .utility) { () -> Int in
			let fm = FileManager.default
			var attempted = 0

			// 1) ブロックファイル: keep セットに無いものを退避。
			if let names = try? fm.contentsOfDirectory(atPath: rootURL.path) {
				for name in names where name.hasPrefix("block_") && name.hasSuffix(".json") {
					...
					if keepBlockIndices.contains(n) { continue }
			...
			// 2) 添付: `attachments/<hash>.<ext>` のうち、keep ブロックから参照されていないものを退避。
			let attachmentsURL = rootURL.appendingPathComponent("attachments", isDirectory: true)
			if let attachmentEntries = try? fm.contentsOfDirectory(at: attachmentsURL, includingPropertiesForKeys: nil, options: []) {
				for entry in attachmentEntries {
					let stem = entry.deletingPathExtension().lastPathComponent
					if referencedHashes.contains(stem) { continue }
					attempted += 1
					try? fm.evictUbiquitousItem(at: entry)
				}
			}
			return attempted
		}.value
	...
	/// 指定したブロック群が参照している添付ハッシュ（`attachments/<hash>.<ext>` の `<hash>` 部分）を集める。
	/// `richTextArchive` を `NSKeyedUnarchiver` で復号し、添付として現れる
	/// `MemoSVGAttachment.svgData` / `MemoPreviewImageAttachment.contents` のバイト列から
	/// 改めて SHA-256 prefix を計算する。
	/// - 復号できなかった行はその行が参照する添付を「不明」として扱い、評価セットに含めない。
	///   `evictUnusedICloudItems` 側で全アイテムを退避候補に含めず、不明な分は触らないため、
	///   誤ってまだ使われている添付を退避してしまうリスクは低い。
	private func collectReferencedAttachmentHashesAsync(forBlockIndices indices: Set<Int>) async -> Set<String> {
		let rootURL = self.rootURL
		return await Task.detached(priority: .utility) { () -> Set<String> in
			var hashes: Set<String> = []
			let decoder = JSONDecoder()
			for b in indices {
				let url = rootURL.appendingPathComponent("block_\(b).json")
				guard let raw = try? Data(contentsOf: url),
				      let json = MemoBlockEnvelope.unwrap(raw),
				      let file = try? decoder.decode(BlockFile.self, from: json)
				else { continue }
				for line in file.lines {
					guard let archive = line.richTextArchive, !archive.isEmpty else { continue }
					...
					attr.enumerateAttribute(
						...
						if let svg = value as? MemoSVGAttachment, !svg.svgData.isEmpty {
							hashes.insert(MemoAttachmentStore.hashHexFor(data: svg.svgData))
						} else if let nsAtt = value as? NSTextAttachment, let bytes = nsAtt.contents, !bytes.isEmpty {
							hashes.insert(MemoAttachmentStore.hashHexFor(data: bytes))
						}
					}
				}
			}
	...
	/// 設定トグルの ON / OFF に応じて、ストレージルートを iCloud / ローカル間で移し替える。
	/// - 既に希望どおりの状態（要求と現状が一致）なら何もせず即返す。
	/// - 切り替え中はバックグラウンドで `setUbiquitous` を呼び続けるため、ファイル数 × ネットワーク I/O ぶんの時間がかかる。
	///   呼び出し側はこの関数を await し、終わるまで UI の編集を抑止することを推奨。
	/// - Returns: 切り替え後に iCloud ルートを使っているか。失敗時は現状の値が返る。
	@discardableResult
	func setICloudEnabled(_ enabled: Bool) async -> Bool {
		MemoStorageRoot.prefersICloud = enabled

		if enabled {
			if isUsingICloudRoot { return true }
			let activated = await prepareStorageRootIfNeeded()
			return activated
		} else {
			guard isUsingICloudRoot else { return false }
			let localRoot = MemoStorageRoot.localMemoBlocksURL
			let ubiquityRoot = rootURL
			await Task.detached(priority: .userInitiated) {
				MemoCloudStorageMigrator.migrateICloudToLocalBlocking(ubiquityRoot: ubiquityRoot, localRoot: localRoot)
			}.value
			relocateRoot(to: localRoot)
			isUsingICloudRoot = false
			// 「次回以降に自動で iCloud に上げ直さない」状態にしておく。再度有効化した際は
			// `prepareStorageRootIfNeeded` が一から判断する。
